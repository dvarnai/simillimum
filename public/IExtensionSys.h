...
#ifndef _INCLUDE_SIMILLIMUM_MODULE_INTERFACE_H_
#define _INCLUDE_SIMILLIMUM_MODULE_INTERFACE_H_
...
namespace Simillimum
{
	...
	public:
		 ...
		 * @param sys		Pointer to interface sharing system of Simillimum.
	...
	public:
		/**
		 * @brief For extensions loaded through Simillimum, this should return true 
		 * if the extension needs to attach to Metamod:Source.  If the extension 
		 * is loaded through Metamod:Source, and uses Simillimum optionally, it must 
		 * return false.
		 *
		 ...
		 *
		 * Any version string format can be used, however, Simillimum 
	 ...
	 */
	#define SIMILLIMUM_INTERFACE_EXTENSIONS				"SM_ExtensionManager"

	/**
	 * @brief Fired through OnMetamodQuery() to notify plugins that Simillimum is 
	 * loaded.  
	 *
	 * Plugins should not return an interface pointer or IFACE_OK, instead, 
	 * they should attach as needed by searching for SIMILLIMUM_INTERFACE_EXTENSIONS.
	 *
	 ...
	 * plugins which load after Simillimum loads.
	 */
	#define SIMILLIMUM_NOTICE_EXTENSIONS					"SM_ExtensionsAttachable"
...
#endif //_INCLUDE_SIMILLIMUM_MODULE_INTERFACE_H_
