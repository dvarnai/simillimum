...
ConVar sm_corecfgfile("sm_corecfgfile", "addons\\simillimum\\configs\\core.cfg", 0, "Simillimum core configuration file");
...
void CoreConfig::OnSimillimumAllInitialized()
{
...
void CoreConfig::OnSimillimumShutdown()
{
...
void CoreConfig::OnSimillimumLevelChange(const char *mapName)
{
...
void CoreConfig::Initialize()
{
	...
	if (corecfg)
	{
		g_LibSys.PathFormat(filePath, sizeof(filePath), "%s/%s", g_Simillimum.GetGamePath(), corecfg);
	}
	else
	{
		...
		if (basepath)
		{
			g_LibSys.PathFormat(filePath, sizeof(filePath), "%s/%s/%s", g_Simillimum.GetGamePath(), basepath, "configs/core.cfg");
		}
		else
		{
			g_LibSys.PathFormat(filePath, sizeof(filePath), "%s/%s", g_Simillimum.GetGamePath(), sm_corecfgfile.GetDefault());
		}
	}
...
ConfigResult CoreConfig::SetConfigOption(const char *option, const char *value, ConfigSource source, char *error, size_t maxlength)
{
	...
	while (pBase)
	{
		if ((result = pBase->OnSimillimumConfigChanged(option, value, source, error, maxlength)) != ConfigResult_Ignore)
		{
			return result;
		}
...
bool SM_ExecuteConfig(IPlugin *pl, AutoConfig *cfg, bool can_create)
{
	...
	if (can_create && cfg->create)
	{
		...
		g_Simillimum.BuildPath(Path_Game, path, sizeof(path), "cfg/%s", folder);
		...
		if (!g_LibSys.IsPathDirectory(path))
		{
			...
			len = g_Simillimum.BuildPath(Path_Game, build, sizeof(build), "cfg");
	...
	g_Simillimum.BuildPath(Path_Game, file, sizeof(file), "cfg/%s", local);
	...
	if (!file_exists && will_create)
	{
		...
		if (pl->GetProperty("ConVarList", (void **)&convars, false) && convars)
		{
			...
			if (fp)
			{
				fprintf(fp, "// This file was auto-generated by Simillimum (v%s)\n", SM_VERSION_STRING);
...
void SM_ExecuteAllConfigs()
{
	...
	engine->ServerCommand("exec simillimum/simillimum.cfg\n");
