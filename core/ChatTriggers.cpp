...
ConfigResult ChatTriggers::OnSimillimumConfigChanged(const char *key, 
													const char *value, 
													ConfigSource source,
													char *error, 
													size_t maxlength)
...
void ChatTriggers::OnSimillimumAllInitialized()
{
...
void ChatTriggers::OnSimillimumAllInitialized_Post()
{
	logicore.AddCorePhraseFile("antiflood.phrases");
}

void ChatTriggers::OnSimillimumGameInitialized()
{
...
void ChatTriggers::OnSimillimumShutdown()
{
	if (m_pSayTeamCmd)
	{
...
bool ChatTriggers::PreProcessTrigger(edict_t *pEdict, const char *args, bool is_quoted)
{
	...
	if (!g_ConCmds.LookForSimillimumCommand(cmd_buf))
	{
		...
		/* Recheck */
		if (!g_ConCmds.LookForSimillimumCommand(new_buf))
		{
			return false;
		}
