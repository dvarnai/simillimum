...
void CExtensionManager::OnSimillimumAllInitialized()
{
...
void CExtensionManager::OnSimillimumShutdown()
{
...
bool CExtensionManager::UnloadExtension(IExtension *_pExt)
{
	...
	if ((pIdentity = pExt->GetIdentity()) != NULL)
	{
		...
		while (glob)
		{
			glob->OnSimillimumIdentityDropped(pIdentity);
...
void CExtensionManager::OnRootConsoleCommand(const char *cmdname, const CCommand &command)
{
		...
		else if (strcmp(cmd, "info") == 0)
		{
			...
			else
			{
				...
				else
				{
					...
					if (pExt->IsExternal())
					{
						rootmenu->ConsolePrint(" Method: Loaded by Metamod:Source, attached to Simillimum");
					}
					else if (pAPI->IsMetamodExtension())
					{
						rootmenu->ConsolePrint(" Method: Loaded by Simillimum, attached to Metamod:Source");
					}
					else
					{
						rootmenu->ConsolePrint(" Method: Loaded by Simillimum");
					}
				}
			}
			return;
	...
	rootmenu->ConsolePrint("Simillimum Extensions Menu:");
