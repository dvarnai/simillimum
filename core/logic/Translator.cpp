...
Translator::Translator() : m_ServerLang(SIMILLIMUM_LANGUAGE_ENGLISH)
{
...
ConfigResult Translator::OnSimillimumConfigChanged(const char *key, 
									  const char *value, 
									  ConfigSource source, 
									  char *error, 
									  size_t maxlength)
...
void Translator::OnSimillimumLevelChange(const char *mapName)
{
...
void Translator::OnSimillimumAllInitialized()
{
...
void Translator::OnSimillimumShutdown()
{
...
void Translator::RebuildLanguageDatabase(const char *lang_header_file)
{
	...
	if ((pServerLang = m_LCodeLookup.retrieve(m_InitialLang)) == NULL)
	{
		...
		m_ServerLang = SIMILLIMUM_LANGUAGE_ENGLISH;
	}
	else
	{
