...
void ShareSystem::OnSimillimumShutdown()
{
	if (m_CoreType)
	{
								   ...
								   SMInterface **pIface)
{
	...
	for (iter=m_Interfaces.begin(); iter!=m_Interfaces.end(); iter++)
	{
		IfaceInfo &info = (*iter);
		iface = info.iface;
		if (strcmp(iface->GetInterfaceName(), iface_name) == 0 || (strcmp(iface_name, "ISourceMod") == 0 && strcmp(iface->GetInterfaceName(), "ISimillimum") == 0))
		{
