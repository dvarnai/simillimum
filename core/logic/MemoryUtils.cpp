...
void MemoryUtils::OnSimillimumAllInitialized()
{
	sharesys->AddInterface(NULL, this);
}

void *MemoryUtils::FindPattern(const void *libPtr, const char *orig_pattern, size_t len)
{
	const unsigned char * pattern = reinterpret_cast<const unsigned char*>(orig_pattern);

	DynLibInfo lib;
	unsigned char *ptr;
	size_t bad_char_skip[UCHAR_MAX + 1], last, scan = 0, hlen;
	...
	ptr = reinterpret_cast<unsigned char *>(lib.baseAddress);
	hlen = lib.memorySize;
 
    for (scan = 0; scan <= UCHAR_MAX; ++scan)
        bad_char_skip[scan] = len;
    ...
    while (hlen >= len)
    {
        for (scan = last; (ptr[scan] == pattern[scan] || pattern[scan] == 0x2A); --scan)
            if (scan == 0)
                return ptr;

		unsigned int skip = (bad_char_skip[ptr[last]] > bad_char_skip[0x2A]?bad_char_skip[0x2A]:bad_char_skip[ptr[last]]);
		hlen -= skip;
		ptr += skip;
    }
