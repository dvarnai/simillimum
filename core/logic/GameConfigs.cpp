...
#define PSTATE_GAMEDEFS_DYNOFFSETS			15
#define PSTATE_GAMEDEFS_DYNOFFSETS_OFFSET	16
	...
	case PSTATE_GAMEDEFS:
		{
			...
			else if (strcmp(name, "Dynamic Offsets") == 0)
			{
				m_ParseState = PSTATE_GAMEDEFS_DYNOFFSETS;
			}
#endif
			else
			{
...
#ifdef PLATFORM_POSIX
	case PSTATE_GAMEDEFS_DYNOFFSETS:
		{
			m_Prop[0] = '\0';
			m_Class[0] = '\0';
			strncopy(m_offset, name, sizeof(m_offset));
			s_TempSig.Reset();
			m_ParseState = PSTATE_GAMEDEFS_DYNOFFSETS_OFFSET;
			break;
		}
#endif
...
SMCResult CGameConfig::ReadSMC_KeyValue(const SMCStates *states, const char *key, const char *value)
{
	...
	} else if (m_ParseState == PSTATE_GAMEDEFS_DYNOFFSETS_OFFSET) {
		if (strcmp(key, PLATFORM_NAME) == 0)
			strncopy(s_TempSig.sig, value, sizeof(s_TempSig.sig));
		else if(strcmp(key, "vtable") == 0)
			strncopy(m_VTable, value, sizeof(m_VTable));
...
SMCResult CGameConfig::ReadSMC_LeavingSection(const SMCStates *states)
{
	...
	case PSTATE_GAMEDEFS_SIGNATURES_SIG:
#ifdef PLATFORM_POSIX
	case PSTATE_GAMEDEFS_DYNOFFSETS_OFFSET:
#endif
		{
			...
			if (addrInBase == NULL)
			{
				...
				if (real_bytes >= 1)
				{
					final_addr = g_MemUtils.FindPattern(addrInBase, (const char*)real_sig, real_bytes);
				}
			}
...
skip_find:
...
#ifdef PLATFORM_POSIX
			if(m_ParseState == PSTATE_GAMEDEFS_DYNOFFSETS_OFFSET && final_addr)
			{
				char m_szVTableSig[256];
				snprintf(m_szVTableSig, sizeof(m_szVTableSig), "_ZTV%d%s", strlen(m_VTable), m_VTable);
				Dl_info info;
				void ** vtable_addr;
				if (dladdr(addrInBase, &info) != 0)
				{
					...
					{
						if (smcore.SymbolsAreHidden())
							vtable_addr = (void**)g_MemUtils.ResolveSymbol(handle, m_szVTableSig);
						else
							vtable_addr = (void**)dlsym(handle, m_szVTableSig);
						dlclose(handle);
					}
				}
				...
				if(vtable_addr)
				{
					...
					if (g_MemUtils.GetLibraryInfo(addrInBase, lib))
					{
						unsigned int length = ((unsigned int)lib.memorySize+(unsigned int)lib.baseAddress-(unsigned int)final_addr)/sizeof(void*);
						for(unsigned int i = 2;i<length;++i)
							if(vtable_addr[i] == final_addr)
							{
								m_Offsets.replace(m_offset, i-2);
								break;
							}
					}
				}
			...
			if(m_ParseState == PSTATE_GAMEDEFS_DYNOFFSETS_OFFSET)
				m_ParseState = PSTATE_GAMEDEFS_DYNOFFSETS;
			else if(m_ParseState == PSTATE_GAMEDEFS_SIGNATURES_SIG)
				m_ParseState = PSTATE_GAMEDEFS_SIGNATURES;
...
void GameConfigManager::OnSimillimumStartup(bool late)
{
...
void GameConfigManager::OnSimillimumAllInitialized()
{
...
void GameConfigManager::OnSimillimumAllShutdown()
{
