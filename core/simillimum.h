...
#ifndef _INCLUDE_SIMILLIMUM_GLOBALHEADER_H_
#define _INCLUDE_SIMILLIMUM_GLOBALHEADER_H_
...
/**
 * @brief Implements Simillimum's global overall management, API, and logic
 */

class SimillimumBase : 
	public ISimillimum,
...
public:
	/**
	 * @brief Initializes Simillimum, or returns an error on failure.
	 */
	bool InitializeSimillimum(char *error, size_t maxlength, bool late);

	/** 
	 * @brief Starts everything Simillimum needs to run
	 */
	void StartSimillimum(bool late);

	/** 
	 * @brief Shuts down all Simillimum components
	 */
	void CloseSimillimum();
...
public: // SMGlobalClass
	ConfigResult OnSimillimumConfigChanged(const char *key, 
										  const char *value, 
										  ConfigSource source, 
										  char *error, 
										  size_t maxlength);
public: // ISimillimum
	...
	const char *GetSimillimumPath() const;
...
extern SimillimumBase g_Simillimum;

#endif //_INCLUDE_SIMILLIMUM_GLOBALHEADER_H_
