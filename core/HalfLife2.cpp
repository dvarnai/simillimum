...
void CHalfLife2::OnSimillimumStartup(bool late)
{
#if SOURCE_ENGINE != SE_DARKMESSIAH
...
#else
	if (strcasecmp(g_Simillimum.GetGameFolderName(), "ship") == 0)
#endif
	{
...
void CHalfLife2::OnSimillimumAllInitialized()
{
...
void CHalfLife2::OnSimillimumAllInitialized_Post()
{
	InitLogicalEntData();
	InitCommandLine();
}
...
void CHalfLife2::InitCommandLine()
{
	...
	g_Simillimum.BuildPath(Path_Game, path, sizeof(path), "../bin/" TIER0_NAME);
	...
	if (m_pGetCommandLine == NULL)
	{
		...
		lib->CloseLibrary();
		g_Simillimum.BuildPath(Path_Game, path, sizeof(path), "../bin/" VSTDLIB_NAME);
		if (!g_LibSys.IsPathFile(path))
		{
