...
ConVar sm_basepath("sm_basepath", "addons\\simillimum", 0, "Simillimum base path (set via command line)");
...
ConfigResult SimillimumBase::OnSimillimumConfigChanged(const char *key, 
									  const char *value, 
									  ConfigSource source, 
									  char *error, 
									  size_t maxlength)
...
bool SimillimumBase::InitializeSimillimum(char *error, size_t maxlength, bool late)
{
	...
	/* Initialize CoreConfig to get the Simillimum base path properly - this parses core.cfg */
	...
	if (getv1 == NULL)
	{
		if (error && maxlength)
		{
			snprintf(error, maxlength, "JIT is too old; upgrade Simillimum");
		}
		ShutdownJIT();
		return false;
	}
	...
	if (!late)
	{
		StartSimillimum(false);
	}

	return true;
}

void SimillimumBase::StartSimillimum(bool late)
{
	SH_ADD_HOOK(IServerGameDLL, LevelShutdown, gamedll, SH_MEMBER(this, &SimillimumBase::LevelShutdown), false);
	...
	while (pBase)
	{
		pBase->OnSimillimumStartup(false);
	...
	while (pBase)
	{
		pBase->OnSimillimumAllInitialized();
		pBase = pBase->m_pGlobalClassNext;
	}
	...
	while (pBase)
	{
		pBase->OnSimillimumAllInitialized_Post();
		pBase = pBase->m_pGlobalClassNext;
	}
	...
	{
		g_Simillimum_Core.OnVSPListening(vsp_interface);
	}
...
bool SimillimumBase::LevelInit(char const *pMapName, char const *pMapEntities, char const *pOldLevel, char const *pLandmarkName, bool loadGame, bool background)
{
	/* If we're not loaded... */
	if (!g_Loaded)
	{
		/* Do all global initialization now */
		StartSimillimum(true);
	}
	...
	while (pBase)
	{
		pBase->OnSimillimumLevelChange(pMapName);
		pBase = pBase->m_pGlobalClassNext;
	}
	...
	while (pBase)
	{
		pBase->OnSimillimumPluginsLoaded();
		pBase = pBase->m_pGlobalClassNext;
	}
...
void SimillimumBase::LevelShutdown()
{
...
bool SimillimumBase::IsMapLoading() const
{
	return m_IsMapLoading;
}

void SimillimumBase::DoGlobalPluginLoads()
{
	...
	g_SMAPI->MetaFactory(SIMILLIMUM_NOTICE_EXTENSIONS, NULL, NULL);
...
size_t SimillimumBase::BuildPath(PathType type, char *buffer, size_t maxlength, const char *format, ...)
{
...
void SimillimumBase::CloseSimillimum()
{
	/* Force a level end */
	LevelShutdown();
	...
	SH_REMOVE_HOOK(IServerGameDLL, LevelInit, gamedll, SH_MEMBER(this, &SimillimumBase::LevelInit), false);
	...
	if (g_Loaded)
	{
		...
		SH_REMOVE_HOOK(IServerGameDLL, LevelShutdown, gamedll, SH_MEMBER(this, &SimillimumBase::LevelShutdown), false);
...
void SimillimumBase::LogMessage(IExtension *pExt, const char *format, ...)
{
...
void SimillimumBase::LogError(IExtension *pExt, const char *format, ...)
{
	IExtensionInterface *pAPI = pExt->GetAPI();
	const char *tag = pAPI->GetExtensionTag();
	char buffer[2048];
...
size_t SimillimumBase::FormatString(char *buffer, size_t maxlength, IPluginContext *pContext, const cell_t *params, unsigned int param)
{
	char *fmt;
...
const char *SimillimumBase::GetSimillimumPath() const
{
	return m_SMBaseDir;
}

const char *SimillimumBase::GetGamePath() const
{
	return g_BaseDir.c_str();
}

unsigned int SimillimumBase::SetGlobalTarget(unsigned int index)
{
...
unsigned int SimillimumBase::GetGlobalTarget() const
{
	return m_target;
}

IDataPack *SimillimumBase::CreateDataPack()
{
	CDataPack *pack;
...
void SimillimumBase::FreeDataPack(IDataPack *pack)
{
	m_freepacks.push(static_cast<CDataPack *>(pack));
}

Handle_t SimillimumBase::GetDataPackHandleType(bool readonly)
{
	//:TODO:
	return 0;
}

const char *SimillimumBase::GetGameFolderName() const
{
	return m_ModDir;
}

ISourcePawnEngine *SimillimumBase::GetScriptingEngine()
{
	return g_pSourcePawn;
}

IVirtualMachine *SimillimumBase::GetScriptingVM()
{
	return NULL;
}

void SimillimumBase::AllPluginsLoaded()
{
	if (!g_Loaded)
	{
		return;
	...
	while (base)
	{
		base->OnSimillimumGameInitialized();
		base = base->m_pGlobalClassNext;
	}
}

time_t SimillimumBase::GetAdjustedTime()
{
	return ::GetAdjustedTime();
}

void SimillimumBase::AddGameFrameHook(GAME_FRAME_HOOK hook)
{
	m_frame_hooks.push_back(hook);
}

void SimillimumBase::RemoveGameFrameHook(GAME_FRAME_HOOK hook)
{
...
void SimillimumBase::ProcessGameFrameHooks(bool simulating)
{
...
size_t SimillimumBase::Format(char *buffer, size_t maxlength, const char *fmt, ...)
{
	size_t len;
	va_list ap;
...
size_t SimillimumBase::FormatArgs(char *buffer,
								 size_t maxlength,
								 const char *fmt,
								 va_list ap)
{
...
void SimillimumBase::AddFrameAction(FRAMEACTION fn, void *data)
{
	::AddFrameAction(FrameAction(fn, data));
}

const char *SimillimumBase::GetCoreConfigValue(const char *key)
{
	return g_CoreConfig.GetCoreConfigValue(key);
}

int SimillimumBase::GetPluginId()
{
	return g_PLID;
}

int SimillimumBase::GetShApiVersion()
{
	int api, impl;
...
bool SimillimumBase::IsMapRunning()
{
