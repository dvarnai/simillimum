...
{
	return g_Simillimum.IsMapLoading();
}

static bool is_map_running()
{
	return g_Simillimum.IsMapRunning();
}
...
{
	g_Simillimum.DoGlobalPluginLoads();
}
...
bool StartLogicBridge(char *error, size_t maxlength)
{
	...
	g_SMAPI->PathFormat(file,
		sizeof(file),
		"%s/bin/simillimum.logic." PLATFORM_LIB_EXT,
		g_Simillimum.GetSimillimumPath());
