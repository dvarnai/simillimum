...
private:
	bool Parse()
	{
		...
		g_Simillimum.BuildPath(Path_SM, m_File, sizeof(m_File), "configs/admin_levels.cfg");
...
void AdminCache::OnSimillimumAllInitialized()
{
...
void AdminCache::OnSimillimumShutdown()
{
	g_Forwards.ReleaseForward(m_pCacheFwd);
	m_pCacheFwd = NULL;
}

void AdminCache::OnSimillimumPluginsLoaded()
{
...
CON_COMMAND(sm_dump_admcache, "Dumps the admin cache for debugging")
{
	...
	g_Simillimum.BuildPath(Path_SM, buffer, sizeof(buffer), "data/admin_cache_dump.txt");
