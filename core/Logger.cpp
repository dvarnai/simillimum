...
ConfigResult Logger::OnSimillimumConfigChanged(const char *key, 
									  const char *value, 
									  ConfigSource source,
									  char *error, 
									  size_t maxlength)
...
void Logger::OnSimillimumStartup(bool late)
{
	InitLogger(m_Mode);
...
void Logger::OnSimillimumAllShutdown()
{
	CloseLogger();
	SH_REMOVE_HOOK(IVEngineServer, LogPrint, engine, SH_STATIC(HookLogPrint), false);
}

void Logger::OnSimillimumLevelChange(const char *mapName)
{
	MapChange(mapName);
}
...
void Logger::_NewMapFile()
{
	...
	while (true)
	{
		g_Simillimum.BuildPath(Path_SM, _filename, sizeof(_filename), "logs/L%02d%02d%03d.log", curtime->tm_mon + 1, curtime->tm_mday, i);
	...
	if (!fp)
	{
		...
		fprintf(fp, "L %s: Simillimum log file started (file \"L%02d%02d%03d.log\") (Version \"%s\")\n", date, curtime->tm_mon + 1, curtime->tm_mday, i, SM_VERSION_STRING);
		fclose(fp);
	}
}
...
void Logger::InitLogger(LoggingMode mode)
{
	...
	char _filename[256];
	g_Simillimum.BuildPath(Path_SM, _filename, sizeof(_filename), "logs/errors_%04d%02d%02d.log", curtime->tm_year + 1900, curtime->tm_mon + 1, curtime->tm_mday);
	...
	case LoggingMode_Daily:
		{
			g_Simillimum.BuildPath(Path_SM, _filename, sizeof(_filename), "logs/L%04d%02d%02d.log", curtime->tm_year + 1900, curtime->tm_mon + 1, curtime->tm_mday);
			m_NrmFileName.assign(_filename);
...
void Logger::LogMessageEx(const char *vafmt, va_list ap)
{
	...
	if (fp)
	{
		if (m_DailyPrintHdr)
		{
			...
			strftime(date, sizeof(date), "%m/%d/%Y - %H:%M:%S", curtime);
			fprintf(fp, "L %s: Simillimum log file session started (file \"L%04d%02d%02d.log\") (Version \"%s\")\n", date, curtime->tm_year + 1900, curtime->tm_mon + 1, curtime->tm_mday, SM_VERSION_STRING);
	 	}
...
void Logger::LogErrorEx(const char *vafmt, va_list ap)
{
	...
	if (fp)
	{
		if (!m_ErrMapStart)
		{
			char date[32];
			strftime(date, sizeof(date), "%m/%d/%Y - %H:%M:%S", curtime);
			fprintf(fp, "L %s: Simillimum error session started\n", date);
...
void Logger::LogFatalEx(const char *msg, va_list ap)
{
	...
	g_Simillimum.BuildPath(Path_Game, path, sizeof(path), "simillimum_fatal.log");
