...
ConVar simillimum_version("simillimum_version", SM_VERSION_STRING, FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY, "Simillimum Version");
...
void RootConsoleMenu::OnSimillimumStartup(bool late)
{
...
void RootConsoleMenu::OnSimillimumAllInitialized()
{
	sharesys->AddInterface(NULL, this);
}

void RootConsoleMenu::OnSimillimumShutdown()
{
...
bool RootConsoleMenu::RegisterConCommandBase(ConCommandBase *pCommand)
{
	...
	/* Override values of convars created by Simillimum convar manager if specified on command line */
...
void RootConsoleMenu::OnRootConsoleCommand(const char *cmdname, const CCommand &command)
{
	if (strcmp(cmdname, "credits") == 0)
	{
		...
		ConsolePrint(" Brian \"420yoloswagboy69\" \"Afronanny\" Simon");
		ConsolePrint(" http://www.simillimum.net/");
	}
	else if (strcmp(cmdname, "version") == 0)
	{
		ConsolePrint(" Simillimum Version Information:");
		ConsolePrint("    Simillimum Version: %s", SM_VERSION_STRING);
...
CON_COMMAND(sm, "Simillimum Menu")
{
