...
void VoteMenuHandler::OnSimillimumAllInitialized()
{
	g_Players.AddClientListener(this);
}

void VoteMenuHandler::OnSimillimumShutdown()
{
	g_Players.RemoveClientListener(this);
}

void VoteMenuHandler::OnSimillimumLevelChange(const char *mapName)
{
	g_next_vote = 0.0f;
}
...
void VoteMenuHandler::OnMenuSelect(IBaseMenu *menu, int client, unsigned int item)
{
	...
	if (item < m_Items)
	{
		...
		if (sm_vote_chat.GetBool() || sm_vote_console.GetBool() || sm_vote_client_console.GetBool())
		{
			...
			{
				int target = SIMILLIMUM_SERVER_LANGUAGE;
...
void VoteMenuHandler::BuildVoteLeaders()
{
	...
	for (unsigned int i=0; i<vote.num_items && i<3; i++)
	{
		...
		len += g_Simillimum.Format(m_leaderList + len, sizeof(m_leaderList) - len, "\n%i. %s: (%i)", i+1, dr.display, vote.item_list[i].count);
	}
}

Simillimum::ResultType VoteMenuHandler::OnTimer(ITimer *pTimer, void *pData)
{
	DrawHintProgress();
