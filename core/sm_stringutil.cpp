...
try_serverlang:
	if (target == SIMILLIMUM_SERVER_LANGUAGE)
	{
	...
	{
		if (target != SIMILLIMUM_SERVER_LANGUAGE && langid != translator->GetServerLanguage())
		{
			target = SIMILLIMUM_SERVER_LANGUAGE;
			goto try_serverlang;
		}
		else if (langid != SIMILLIMUM_LANGUAGE_ENGLISH)
		{
			if (pPhrases->FindTranslation(key, SIMILLIMUM_LANGUAGE_ENGLISH, &pTrans) != Trans_Okay)
			{
...
try_again:
				...
				{
					if (target != SIMILLIMUM_SERVER_LANGUAGE && lang_id != translator->GetServerLanguage())
					{
						target = SIMILLIMUM_SERVER_LANGUAGE;
						goto try_again;
					}
					else if (lang_id != SIMILLIMUM_LANGUAGE_ENGLISH)
					{
						if (pPhrases->FindTranslation(key, SIMILLIMUM_LANGUAGE_ENGLISH, &trans) != Trans_Okay)
						{
							if (pFailPhrase != NULL)
							{
								*pFailPhrase = key;
		...
		case 't':
			{
				...
				bool error;
				size_t res;
				cell_t target = g_Simillimum.GetGlobalTarget();
				pCtx->LocalToString(params[arg++], &key);
				res = Translate(buf_p, llen, pCtx, key, target, params, &arg, &error);
				if (error)
				{
