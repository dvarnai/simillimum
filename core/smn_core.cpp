...
static cell_t ThrowError(IPluginContext *pContext, const cell_t *params)
{
	...
	g_Simillimum.FormatString(buffer, sizeof(buffer), pContext, params, 1);
...
static cell_t sm_LogAction(IPluginContext *pContext, const cell_t *params)
{
	char buffer[2048];
	g_Simillimum.SetGlobalTarget(SIMILLIMUM_SERVER_LANGUAGE);
	g_Simillimum.FormatString(buffer, sizeof(buffer), pContext, params, 3);
...
static cell_t LogToFile(IPluginContext *pContext, const cell_t *params)
{
	...
	g_Simillimum.BuildPath(Path_Game, path, sizeof(path), "%s", file);
