...
void DBManager::OnSimillimumAllInitialized()
{
	HandleAccess sec;
	...
	g_Simillimum.BuildPath(Path_SM, m_Filename, sizeof(m_Filename), "configs/databases.cfg");
...
void DBManager::OnSimillimumLevelChange(const char *mapName)
{
	SMCError err;
...
void DBManager::OnSimillimumShutdown()
{
	KillWorkerThread();
...
void DBManager::OnSimillimumIdentityDropped(IdentityToken_t *pToken)
{
	s_pAddBlock = pToken;
