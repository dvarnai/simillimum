...
ConVar sm_menu_sounds("sm_menu_sounds", "1", 0, "Sets whether Simillimum menus play trigger sounds");
...
void MenuManager::OnSimillimumAllInitialized()
{
...
void MenuManager::OnSimillimumAllShutdown()
{
...
ConfigResult MenuManager::OnSimillimumConfigChanged(const char *key,
												   const char *value,
												   ConfigSource source,
												   char *error,
												   size_t maxlength)
...
void MenuManager::OnSimillimumLevelChange(const char *mapName)
{
	if (m_SelectSound.size() > 0)
	{
