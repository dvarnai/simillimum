...
ConVar sm_show_activity("sm_show_activity", "13", FCVAR_SPONLY, "Activity display setting (see simillimum.cfg)");
							...
							cell_t fmt_param)
{
	...
	if (client != 0)
	{
		...
		{
			g_Simillimum.SetGlobalTarget(client);
			g_Simillimum.FormatString(buffer, sizeof(buffer), pContext, params, fmt_param);
	...
	for (int i=1; i<=maxClients; i++)
	{
		...
		AdminId id = pPlayer->GetAdminId();
		g_Simillimum.SetGlobalTarget(i);
		if (id == INVALID_ADMIN_ID
			|| !g_Admins.GetAdminFlag(id, Admin_Generic, Access_Effective))
		{
