...
/* Simillimum <==> SourceHook type conversion functions */

SourceHook::ProtoInfo::CallConvention GetSHCallConvention(Simillimum::CallConvention cv)
{
	if (cv < Simillimum::CallConv_ThisCall || cv > Simillimum::CallConv_Cdecl)
	{
...
Simillimum::CallConvention GetSMCallConvention(SourceHook::ProtoInfo::CallConvention cv)
{
	assert(cv >= SourceHook::ProtoInfo::CallConv_ThisCall || cv <= SourceHook::ProtoInfo::CallConv_Cdecl);

	return (Simillimum::CallConvention)(cv - 1);
}

SourceHook::PassInfo::PassType GetSHPassType(Simillimum::PassType type)
{
	if (type < Simillimum::PassType_Basic || type > Simillimum::PassType_Object)
	{
...
Simillimum::PassType GetSMPassType(int type)
{
	/* Simillimum doesn't provide an Unknown type so we it must be an error */
	assert(type >= SourceHook::PassInfo::PassType_Basic && type <= SourceHook::PassInfo::PassType_Object);

	return (Simillimum::PassType)(type - 1);
}

void GetSMPassInfo(Simillimum::PassInfo *out, const SourceHook::PassInfo *in)
{
	out->size = in->size;
	out->flags = in->flags;
